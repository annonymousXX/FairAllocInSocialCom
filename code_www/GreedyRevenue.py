...
def algo_greedy_rev(file, L, R):
    v = np.load(file)
    users = v.shape[0]
    items = v.shape[1]

    T = users*L
    #Lets assume c1 to be 10
    count_to_users = np.zeros(items)
    count_to_items = np.zeros(users)
    ...
    while(T>0):
        ...
        if(np.amax(v)>-1):
            ...
            count_to_items[i] += 1
            v[i][j] = -1
            if(count_to_users[j] == R):
                for k in range(0,users):
                    v[k][j] = -1
            if(count_to_items[i] == L):
    ...
    for i in range(0,users):
        if(count_to_items[i] < L):
            remaing_items_count = int(L - count_to_items[i])
            store_feature = np.array([1 if j not in A[i] else 0 for j in range(items)])
            store_feature = store_feature * v[i]
            ind = np.argpartition(store_feature, -remaing_items_count)[-remaing_items_count:]
            for j in ind:
                A[i].append(j)


    #print("Algorithm Rev_Greedy has terminated.")
    time_taken = (time.time() - start_time)
    fin = np.array(A)
    fin = swap_for_cardinality(fin,v,R)
    print(f"Rev_Greedy Time taken ->{time_taken} seconds")
...
if __name__ == '__main__':
    ...
    start_time = time.time()
    fin = algo_greedy_rev(fileName, L, R1)
    time_taken = (time.time() - start_time)
