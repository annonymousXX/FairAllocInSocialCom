...
def GreedyNash(file, L, R):
    ...
    for user_idx in range(0,users):
        ...
        count_to_alloc[user_idx] += 1
        
    prod_idx = 0
    while (count_to_buy[prod_idx] == 0):
        prod_idx = prod_idx + 1
    while (prod_idx < items):
        user_indices = np.where(count_to_alloc<L)[0]
        user_feature = np.array([1 if (i in user_indices and  prod_idx not in A[i]) else 0 for i in range(users)])
        user_feature = user_feature * v[:,prod_idx]  #check this
        #if (np.sum(user_feature) ==0):
            #print("hi, could not find any user for product", prod_idx)
        utill_gap = np.log(utility+user_feature)-np.log(utility)
        if (np.sum(utill_gap) > 0 ):
            idx = np.argmax(utill_gap)
            count_to_buy[prod_idx] -= 1
            A[idx].append(prod_idx)
            utility[idx] += user_feature[idx]
            count_to_alloc[idx] += 1 
            if (count_to_buy[prod_idx] == 0):
                while (prod_idx < items and count_to_buy[prod_idx] == 0 ):
                    prod_idx = prod_idx + 1
                        
        else:
            #print("could not find any user for product", prod_idx)
            #print(utill_gap)
            prod_idx = prod_idx + 1
            while (prod_idx < items and count_to_buy[prod_idx] == 0):
                prod_idx = prod_idx + 1
